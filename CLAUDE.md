# nanogen -- Agent Reference

## Architecture

<!-- TODO: describe source layout when project has code -->

## Dev Server

```bash
# TODO: configure dev server command in .claude/zskills-config.json
```

The port is determined automatically by `scripts/port.sh`: **8080** for the main repo (`/workspaces/nanogen`), a **deterministic unique port** for each worktree (derived from the project root path). Run `bash scripts/port.sh` to see your port. Override with `DEV_PORT=NNNN` env var if needed.
...
```js
// TODO: configure auth bypass if the app has auth
```
...
```bash
# TODO: configure unit test command in .claude/zskills-config.json    # Unit tests only -- fast, use while working
# TODO: configure full test command in .claude/zskills-config.json    # ALL suites -- use before committing
```

**`# TODO: configure full test command in .claude/zskills-config.json` must pass before every commit.** When reporting test
...
`# TODO: configure full test command in .claude/zskills-config.json > .test-results.txt 2>&1` -- then read the file to
...
<!-- TODO: list test file patterns when project has tests -->
